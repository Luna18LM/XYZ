...
(function ($) {
    ...
    $.fn.validationEngine = function (settings) {
        ...
        $("body").on("click", "div[class$='formError']", function () {	 // REMOVE BOX ON CLICK
    ...
    $.validationEngine = {
        ...
        validateCall: function (caller, rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
            ...
            for (var i = 0; i < rules.length; i++) {
                switch (rules[i]) {
                    ...
                    case "funcCall":
                        ...
                        break;
                    case "dontmatch":
                        _dontmatch(caller, rules, i);
                        break;
                    case "samevalues":
                        _samevalues(caller, rules, i);
                        break;
                    case "between":
                        _between(caller, rules, i);
                        break;
                    case "min":
                    ...
                    case "expirationdate":
                        ...
                        break;
                    case "html":
                        _validateHtml(caller, rules);
                        break;
                    //case "skuid":
                    //    _skuid(caller, rules, i);
                    //    break;
                    default:
                }
            }
            ...
            function _required(caller, rules) { // VALIDATE BLANK FIELD
                var callerType = caller.type; //$(caller).attr("type");
                if (callerType == "text" || callerType == "password" || callerType == "textarea") {
                    if ($(caller).attr("id").indexOf("summer") >= 0) {
                        if (!$(caller).code()) {
            ...
            function _allrequired(caller, rules) { // VALIDATE BLANK FIELD
                var callerType = $(caller.children).attr("type"); //$(caller).attr("type");
                if (callerType == "radio" || callerType == "checkbox") {
                    callerName = $(caller.children).attr("name");
            ...
            function _customRegex(caller, rules, position) {		 // VALIDATE REGEX RULES
                ...
                if (customRule == "onlyLetterSp" || customRule == "onlyLetterNumber" || customRule == "dateFormat") {
                    if (caller.value == "") {
                        return true;
                    }
                ...
                if (customRule == "url") {
                    if ($("#ContentPlaceHolder_ContentPlaceHolder2_chkUploadFile").is(":checked")) {
                        return true;
                    }
                }
                if (customRule == "onlyNumberSp") {
                    if (caller.value == "") {
                        //caller.value = 0;
                        return true;
                    }
            ...
            function _validateHtml(caller, rules) { // added by Jorge RC for validate script tags in forms
                if ($(caller).val().match(/<(\w+)((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/)) {
                    $.validationEngine.isError = true;
                    promptText += "* Information incorrect <br/>"//$.validationEngine.settings.allrules["html"].alertText + "information incorrect <br />";
                }
            }            

            function _skuid(caller, rules, position) { // added by MReaño for value skuid Search
                if (!/^[\w _.-]+$/.test($(caller).val())) {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["Sku Id"].alertText + " <br />";
                } else {
                    $.validationEngine.isError = false;
                    promptText += " <br />";
                }
            ...
            function _between(caller, rules, position) {
                var values = rules[position + 1].split('|');
                if ($(caller).val() < values[0] || $(caller).val() > values[1]) {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["between"].alertText + "<br />";
                }

            }

            /*NEED TO BE FIXE!!!!!!!!*/
            function _dontmatch(caller, rules, position) {
                var text = rules[position + 1].split('|');
                if ($(caller).text() == text[0] || $("[id$=" + text[1] + "]").val() == '') {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["dontmatch"].alertText + "<br />";
                }
            }


            function _samevalues(caller, rules, position) {
                var input = rules[position + 1];
                if ($(caller).val() != $('[id$=' + input + ']').val()) {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["samevalues"].alertText + "<br />";
                }
